...
export function registerInsightsCommand(program: Command) {
  program
    .command("insights")
    ...
    .action(async (opts) => {
      try {
        ...
        console.log(
          `${"キャンペーン".padEnd(maxName)}  ${"表示回数".padStart(8)}  ${"クリック".padStart(8)}  ${"費用".padStart(10)}  CTR`
        );
        console.log("-".repeat(maxName + 44));
        for (const r of rows) {
          console.log(
            `${r.campaignName.padEnd(maxName)}  ${String(r.impressions).padStart(8)}  ${String(r.clicks).padStart(8)}  ${r.cost.padStart(10)}  ${r.ctr}`
          );
        }
      } catch (err) {
