 ...
 */
export function checkBudgets(campaigns, thresholdYen) {
...
export function registerBudgetCheckCommand(program) {
    program
        ...
        try {
            ...
            console.log(`\x1b[33m⚠️  予算上限未設定または上限超過のキャンペーン（閾値: ¥${thresholdYen.toLocaleString()}/日）:\x1b[0m`);
            for (const alert of result.alerts) {
                if (alert.reason === "unlimited") {
                    console.log(`  \x1b[31m• ${alert.name} (ID: ${alert.id}) — 予算上限未設定（unlimited）\x1b[0m`);
                }
                else {
                    console.log(`  \x1b[33m• ${alert.name} (ID: ${alert.id}) — ¥${alert.budgetYen.toLocaleString()}/日（閾値超過）\x1b[0m`);
                }
            }
