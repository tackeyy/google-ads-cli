...
export function registerCreateCommand(program) {
    program
        ...
        try {
            ...
            const budgetResourceName = await client.createCampaignBudget("Budget_zeimu-search-wl-v1", 1_667_000_000 // ¥1,667/日 (micros)
            );
            const budgetId = budgetResourceName.split("/").pop();
            console.log(`  ✅ 予算作成: ID=${budgetId}`);
            // ── Step 2: キャンペーン ──────────────────────────────────
            console.log("Step 2/7: キャンペーンを作成中...");
            const campaignResourceName = await client.createCampaign("zeimu-search-wl-v1", budgetResourceName);
            ...
            const kw1ResourceNames = await client.addKeywords(adGroup1ResourceName, [
                ...group1PhraseKeywords.map((text) => ({ text, matchType: "PHRASE" })),
                ...group1ExactKeywords.map((text) => ({ text, matchType: "EXACT" })),
            ]);
            ...
            const kw2ResourceNames = await client.addKeywords(adGroup2ResourceName, [
                ...group2PhraseKeywords.map((text) => ({ text, matchType: "PHRASE" })),
            ]);
            console.log(`  ✅ キーワード ${kw2ResourceNames.length} 個設定完了`);
            // ── Step 6: 広告3本 × グループ1 ─────────────────────────
            console.log("Step 6/7: 広告グループ1に広告3本を作成中...");
            const ad1aResourceName = await client.createResponsiveSearchAd(adGroup1ResourceName, ["仕訳作業を90%削減", "2時間30分→15分に短縮", "税理士向けAI｜先行登録受付中"], [
            ...
            const ad1bResourceName = await client.createResponsiveSearchAd(adGroup1ResourceName, ["スタッフ採用コストの1/10以下", "AIが月次決算チェックを代行", "税理士向けAI｜無料で先行登録"], [
            ...
            const ad1cResourceName = await client.createResponsiveSearchAd(adGroup1ResourceName, ["税理士向けAI 先行登録受付中", "月次決算を自動化｜無料体験", "仕訳90%削減｜AIパートナー"], [
            ...
            const ad2aResourceName = await client.createResponsiveSearchAd(adGroup2ResourceName, ["仕訳作業を90%削減", "2時間30分→15分に短縮", "税理士向けAI｜先行登録受付中"], [
                "月次決算の仕訳チェックをAIが自動処理。税理士の確認作業を最大80%削減します。",
                "freeeと連携するだけ。5分で使い始められる税理士向けAIパートナー。",
            ], "https://zeimu.ai/lp/?utm_source=google&utm_medium=paid_search&utm_campaign=wl-acquisition&utm_content=ad-a-efficiency");
            console.log(`  ✅ 広告A (数字訴求): ${ad2aResourceName.split("/").pop()}`);
            const ad2bResourceName = await client.createResponsiveSearchAd(adGroup2ResourceName, ["スタッフ採用コストの1/10以下", "AIが月次決算チェックを代行", "税理士向けAI｜無料で先行登録"], [
                "200社の顧問先管理もAIがサポート。転記作業を1/5に削減した実績。",
                "ISMS認証取得済み。税理士事務所のデータを安全に処理します。",
            ], "https://zeimu.ai/lp/?utm_source=google&utm_medium=paid_search&utm_campaign=wl-acquisition&utm_content=ad-b-cost");
            console.log(`  ✅ 広告B (コスト訴求): ${ad2bResourceName.split("/").pop()}`);
            const ad2cResourceName = await client.createResponsiveSearchAd(adGroup2ResourceName, ["税理士向けAI 先行登録受付中", "月次決算を自動化｜無料体験", "仕訳90%削減｜AIパートナー"], [
                "税理士の月次決算チェック・異常値検出をAIが自動化。顧問先200社分を管理。",
                "メールアドレスだけで登録可能。正式版は採用コストの1/10以下の価格予定。",
            ], "https://zeimu.ai/lp/?utm_source=google&utm_medium=paid_search&utm_campaign=wl-acquisition&utm_content=ad-c-earlyaccess");
